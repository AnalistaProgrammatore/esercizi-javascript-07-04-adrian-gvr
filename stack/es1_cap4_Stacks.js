...
function checkMissingParenthesis(espressione){   // la funzione che verifica la corrispondenza tra parentesi aperte e parentesi chiuse
    ...
    //if (pStack.length() > 0) missingCloseP = pStack.pop();
    
    
    if (missingOpenP !== undefined) console.log(`nella posizione ${missingOpenP} c'è una parentesi che non è mai stata aperta.`);
    if (missingCloseP !== undefined) console.log(`nella posizione ${missingCloseP} c'è una parentesi aperta che non è mai stata chiusa.`);
